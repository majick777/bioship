...
<div class="login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="loginform" id="loginform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'login' ); ?>" method="post">
		<p>
			...
			<?php $usernamefield = 'Username';
			$tml = get_option('theme_my_login'); $email_login = $tml['email_login'];
			if ($email_login) {$usernamefield = 'Email/Username';} ?>
			<input type="text" name="log" id="user_login<?php $template->the_instance(); ?>" class="input" placeholder="<?php _e($usernamefield); ?>" value="<?php $template->the_posted_value( 'log' ); ?>" size="20" />
		</p>
		<p>
			<!-- <label for="user_pass<?php $template->the_instance(); ?>"><?php _e( 'Password' ); ?></label> -->
			<input type="password" name="pwd" id="user_pass<?php $template->the_instance(); ?>" class="input" placeholder="<?php _e('Password'); ?>" value="" size="20" />
		</p>
