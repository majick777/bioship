...
*/
?>
<h2><?php printf( __( 'Get your own %s account in seconds', 'bioship' ), $current_site->site_name ); ?></h2>
...
<form id="setupform" method="post" action="<?php $template->the_action_url( 'register' ); ?>">
	...
	<label for="user_name<?php $template->the_instance(); ?>"><?php _e( 'Username:', 'bioship' ); ?></label>
	...
	<span class="hint"><?php _e( '(Must be at least 4 characters, letters and numbers only.)', 'bioship' ); ?></span>

	<label for="user_email<?php $template->the_instance(); ?>"><?php _e( 'Email&nbsp;Address:', 'bioship' ); ?></label>
	...
	<span class="hint"><?php _e( 'We send your registration email to this address. (Double-check your email address before continuing.)', 'bioship' ); ?></span>
	...
	<?php } else { ?>
		...
		<label class="checkbox" for="signupblog"><?php _e( 'Gimme a site!', 'bioship' ); ?></label>
		<br />
		...
		<label class="checkbox" for="signupuser"><?php _e( 'Just a username, please.', 'bioship' ); ?></label>
	<?php } ?>
	</p>

	<p class="submit"><input type="submit" name="submit" class="submit" value="<?php esc_attr_e( 'Next', 'bioship' ); ?>" /></p>
</form>
