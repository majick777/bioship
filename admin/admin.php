...
// 2.2.0: use file hierarchy for metabox.php
// include(dirname(__FILE__).'/metabox.php');
$metabox = bioship_file_hierarchy( 'file', 'metabox.php', $vthemedirs['admin'] );
if ( $metabox ) {
	include $metabox;
	define( 'THEMEMETABOX', true );
}
...
// ---------------------------------------
// Usage: ?themedebug=(themeoptions/options/backupoptions/backup)
if ( !function_exists( 'bioship_admin_echo_setting_values' ) ) {
 ...
 function bioship_admin_echo_setting_values() {
	...
	if ( !isset( $_REQUEST['themedump'] ) || !in_array( $_REQUEST['themedump'], $valid ) ) {
		return;
	}
	...
	if ( ($_REQUEST['themedump'] == 'backupoptions') || ( 'backup' == $_REQUEST['themedump'] ) ) {
...
if ( !function_exists( 'bioship_admin_notices' ) ) {
 function bioship_admin_notices() {
	...
	// 2.2.0: added isset check inside function
	if ( !isset( $vthemeadminmessages ) ) {
		$vthemeadminmessages = array();
	}
	if ( count( $vthemeadminmessages) > 0 ) {
		foreach ( $vthemeadminmessages as $message ) {
			echo '<div class="update message" style="padding:3px 10px;margin:0 0 10px 0;">';
			echo esc_attr( $message ) . '</div>';
		}
	}
 }
}
...
if ( !function_exists( 'bioship_admin_check_filesystem_credentials' ) ) {
 function bioship_admin_check_filesystem_credentials( $url, $method, $context, $extrafields ) {
	...
	if ( !WP_Filesystem( $credentials ) ) {
		request_filesystem_credentials( $url, $method, true, $context, $extrafields );
		return false;
	}
	return true;
 }
 ...
 function bioship_admin_get_directory_files( $dir, $recursive = true, $basedir = '' ) {
	...
	foreach ( $files as $key => $value ) {
		if ( !in_array( $value, array( '.', '..' ) ) ) {
			...
			if ( is_dir( $path ) ) { // do not combine with the next line or fails
				...
				// strip basedir and add to subarray to separate list
...
// ------------------------
// 2.2.0: added for filtering child themes from updates transient
if ( !function_exists( 'bioship_updates_transient_filter' ) ) {

 add_filter( 'pre_set_site_transient_update_themes', 'bioship_updates_transient_filter' );

 function bioship_updates_transient_filter( $transient ) {

    $themes = wp_get_themes();
    foreach ( $themes as $slug => $theme ) {
    	// --- check if this is a Bioship Child Theme ---
    	if ( ( 'bioship' == $theme->template ) && ( 'bioship' != $theme->stylesheet ) ) {
    		// --- set the update response to null if so ---
    		// (to prevent update conflicts with Child Theme and WP Repo Theme names)
    		$transient->response[$slug] = null;
			$item = array(
				'theme'        => 'my-theme',
				'new_version'  => '1.0.0',
				'url'          => '',
...
if ( !function_exists('bioship_theme_update_checker' ) ) {
 ...
 function bioship_theme_update_checker() {
	...
	if ( THEMEWPORG ) {
		...
		// 1.8.5: maybe disable directory clearing to keep any installed theme bundles ?
...
// ---------------------------------
// ### MARKER
// TODO: check if $updatemessage is actually used anywhere?!
if ( !function_exists( 'bioship_admin_theme_updates_available' ) ) {
 function bioship_admin_theme_updates_available() {
	...
	if ( !is_object( $updatestransient ) || !property_exists( $updatestransient, 'response' ) ) {
		return $updatehtml;
	}
	...
	if ( THEMECHILD ) {
		if ( isset( $updates[$themeslug] ) ) {
			...
			// 2.2.0: use add_query_arg to generate upgrade URL
			$update = $updates[$themeslug];
			$upgrade_url = add_query_arg( 'action', 'upgrade-theme', admin_url( 'update.php' ) );
			$upgrade_url = add_query_arg( 'theme', urlencode( $themeslug ), $upgrade_url );
			$url = wp_nonce_url( $upgrade_url, 'upgrade-theme_' . $themeslug );
			...
			if ( !empty($update['package'] ) ) {
				$updatemessage = esc_html( __( 'New Child Theme available.', 'bioship' ) );
				$updatemessage .= '<br><a href="' . esc_url( $update['url'] ) . '" title="' . esc_attr( $themedisplayname ) . '" target="_blank" onclick="' . $onclick . '">';
				$updatemessage .= esc_html( __( 'Update to', 'bioship' ) ) . ' ' . $update['new_version'] . '</a>';
				$updatehtml .= $updatemessage;
	...
	if ( isset( $updates[$themeslug] ) ) {
		...
		if ( version_compare( $parenttheme['Version'], $update['new_version'], '<' ) ) {
			...
			$onclick = 'onclick="if (confirm(\'' . esc_js( $message ) . '\')) {return true;} return false;"';
			if ( !empty( $update['package'] ) ) {
				$updatemessage = esc_html( __( 'New Framework version available!', 'bioship' ) );
				$updatemessage .= '<br><a href="' . esc_url( $url ) . '" title="' . esc_attr( $themedisplayname ) . '" target="_blank">';
				$updatemessage .= esc_html( __( 'Update to','bioship' ) ) . ' ' . esc_attr( $update['new_version'] ) .'</a>';
				$updatehtml .= $updatemessage;
				$updatehtml .= '<!-- THEME UPDATE AVAILABLE -->';
			}
			...
			// if ( THEMEWPORG ) {
			//	$url = 'http://themes.svn.wordpress.org/bioship/'.$update['new_version'].'/readme.txt';
			// } else {
			//	$url = 'https://bioship.space/download/packages/readme.txt';
			// }

			// $readme = wp_remote_get( $url );
			// if ( !is_wp_error( $readme ) && stristr( $readme['body'], 'Upgrade Notice' ) ) {
				// $reader = bioship_file_hierarchy( 'file', 'reader.php', $vthemedirs['includes'] );
				// if ( $reader ) {
				//	include $reader;
...
// TODO: also show on theme and dashboard pages ?
...
if ( !function_exists( 'bioship_admin_freemius' ) ) {
 function bioship_admin_freemius() {
	...
	// 2.2.0: moved checks inside function
	$bioship_freemius = false;
    ...
    if ( !is_object( $bioship_freemius ) ) {
		...
		if ( THEMETITAN ) {
			...
			// 2.2.0: fix to the fix of the mismatching page slug
			$menu['slug']		= 'theme-info';
			$menu['parent']		= array( 'slug' => 'themes.php' );
			$menu['first-path']	= 'themes.php?page=theme-info&welcome=true';
		}
		...
		// 2.2.0: debug freemius object
		bioship_debug( "Freemius Settings", $bioship_settings );
		bioship_debug( "Freemius Object", $bioship_freemius );
    }
...
// 2.2.0: set values within load function
...
// License Change Updater
// ----------------------
// (no plans so does not do anything yet)
...
if ( !function_exists( 'bioship_muscle_add_bioship_dashboard_feed_widget' ) ) {
	...
	// 2.2.0: add to action anyway without these unnecessary checks
	// $requesturi = $_SERVER['REQUEST_URI'];
	// if ( ( preg_match( '|index.php|i', $requesturi ) )
	//  || ( '/wp-admin/' == substr( $requesturi, -( strlen( '/wp-admin/' ) ) ) )
	//  || ( '/wp-admin/network/' == substr( $requesturi, -( strlen( '/wp-admin/network/' ) ) ) ) ) {
	...
	function bioship_muscle_add_bioship_dashboard_feed_widget() {
		...
		if ( 'dashboard' == $screen->id ) {
			bioship_debug( "This is a dashboard screen." );
		}

		// --- check permissions ---
		if ( current_user_can( 'manage_options' ) || current_user_can( 'update_themes' ) || current_user_can( 'edit_theme_options' ) ) {
...
if ( !function_exists( 'bioship_muscle_bioship_dashboard_feed_widget' ) ) {
 function bioship_muscle_bioship_dashboard_feed_widget( $displayupdates = true, $displaylinks = false ) {
	...
	// -------------------------
	// 2.2.0: do not attempt to re-include admin.php!
	if ( $displayupdates ) {
		// phpcs:ignore WordPress.Security.OutputNotEscaped,WordPress.Security.OutputNotEscapedShortEcho
		echo bioship_admin_theme_updates_available();
	}
	...
	if ( THEMEDEBUG ) {
		...
		$feed = trim( get_transient( 'bioship_feed' ) );
	}

	// --- fetch feed ---
	if ( !$feed || ( '' == $feed ) ) {
		$rssfeed = fetch_feed( $rssurl );
		bioship_debug( "BioShip RSS URL", $rssurl );
		bioship_debug( "BioShip RSS Feed", $rssfeed );
		$feeditems = 4;
		$feed = bioship_muscle_process_rss_feed( $rssfeed, $feeditems );
		if ( $feed && ( '' != $feed ) ) {
			set_transient('bioship_feed', $feed, ( 24 * 60 * 60 ) );
		}
	}
	...
	if ( $displaylinks ) {
		echo '<center><b><a href="' . esc_url( THEMEHOMEURL . '/documentation/' ) . '" class="themefeedlink" target="_blank">';
			echo esc_html( __( 'Documentation', 'bioship' ) ) . '</a></b> | ';
		echo '<b><a href="' . esc_url( THEMEHOMEURL . '/development/' ) . '" class="themefeedlink" target="_blank">';
			echo esc_html( __( 'Development', 'bioship' ) ) . '</a></b> | ';
		echo '<b><a href="' . esc_url( THEMEHOMEURL . '/extensions/' ) . '" class="themefeedlink" target="_blank">';
			echo esc_attr( __( 'Extensions', 'bioship' ) ) . '</a></b></center><br>';
	}
	...
	echo '<div id="bioshipfeed">';
		$news_url = $baseurl . '/category/news/';
		echo "<div style='float:right;'>&rarr;<a href='". esc_url( $news_url ) . "' class='themefeedlink' target=_blank> ";
		echo esc_html( __( 'More', 'bioship' ) ) . '...</a></div>';
		if ( '' != $feed ) {
			// phpcs:ignore WordPress.Security.OutputNotEscaped,WordPress.Security.OutputNotEscapedShortEcho
			echo $feed;
		} else {
			echo esc_html( __( 'Feed currently unavailable.', 'bioship' ) );
			delete_transient( 'bioship_feed' );
		}
	echo '</div>';
 }
 ...
 function bioship_muscle_process_rss_feed( $rss, $feeditems ) {
	...
	if ( count($rssitems) > 0 ) {
		...
		foreach ( $rssitems as $item ) {
			...
			$processed .= 'title="Posted ' . esc_attr( $item->get_date( 'j F Y | g:i a' ) ) . '" class="themefeedlink">';
...
if ( !function_exists( 'bioship_admin_right_now_content_table_end' ) ) {
 ...
 function bioship_admin_right_now_content_table_end() {
	...
	// --- check settings ---
	$modify = false;
	if ( isset( $vthemesettings['cptsrightnow'] ) ) {
		$modify = $vthemesettings['cptsrightnow'];
	}
	...
	foreach( $posttypes as $posttype ) {
		...
		if ( 1 == $postcount ) {
			$text = $postcount . ' ' . $singular;
		} else {
			$text = $postcount . ' ' . $label;
		}
		echo '<tr><td class="first num b b-' . esc_attr( $posttype->name ) . '">';
		if ( current_user_can( 'edit_posts' ) ) {
			$edit_url = 'edit.php?post_type=' . esc_attr( $posttype->name );
			echo '<a href="' . esc_url( $edit_url ) . '">' . esc_attr( $num ) . '</a>';
		} else {
			echo esc_attr( $num );
		}
		echo '</td><td class="text t ' . esc_attr( $posttype->name ) . '">';
		if ( current_user_can( 'edit_posts' ) ) {
			echo '<a href="' . esc_url( $edit_url ) . '">' . esc_attr( $text ) . '</a>';
		} else {
			echo esc_attr( $text );
		}
		echo '</td></tr>';
	...
	foreach ( $taxonomies as $taxonomy ) {
		...
		if ($termcount == 1) {
			$text = $termcount.' '.$singular;
		} else {
			$text = $termcount.' '.$label;
		}
		// 2.2.0: fix to esc_attr type
		echo '<tr><td class="first b b-' . esc_attr( $taxonomy->name ) . '">';
		if ( current_user_can( 'manage_categories' ) ) {
			$edit_url = 'edit-tags.php?taxonomy=' . esc_attr( $taxonomy->name );
			$num = '<a href="' . esc_url( $edit_url ) . '">' . esc_attr( $num ) . '</a>';
		} else {
			echo esc_attr( $num );
		}
		echo '</td><td class="t '.esc_attr($taxonomy->name).'">';
		if ( current_user_can( 'manage_categories' ) ) {
			$text = '<a href="' . esc_url( $edit_url ) . '>' . esc_attr( $text ) . '</a>';
		} else {
			echo esc_attr( $text );
		}
		echo '</td></tr>';
...
if ( !function_exists( 'bioship_admin_thumbnail_column' ) ) {
 ...
 function bioship_admin_thumbnail_column($cols){
	...
	if ( !$thumbcols || !is_array( $thumbcols ) || ( count($thumbcols ) == 0 ) ) {
		return $cols;
	}
...
if ( !function_exists( 'bioship_admin_display_thumbnail_column' ) ) {
 function bioship_admin_display_thumbnail_column( $col, $postid ) {
	...
	if ( 'post_thumb' == $col ) {
		...
		// 2.2.0: use get_the_post_thumbnail instead of the_post_thumbnail
		echo get_the_post_thumbnail( $postid, 'admin-thumb' );
	}
 }
}
...
if ( !function_exists( 'bioship_admin_thumbnail_column_width' ) ) {
 function bioship_admin_thumbnail_column_width() {
	...
	// 2.2.0: added isset check for width
	if ( !isset( $width ) || $width < 75 ) {
		$width = 75;
	}
...
if ( !function_exists( 'bioship_admin_theme_options_page_redirect' ) ) {
 ...
 function bioship_admin_theme_options_page_redirect($updated = '') {
	...
	// 2.2.0: fix to theme tools path to use themes.php
	$optionsurl = admin_url( 'admin.php' );
	if ( THEMETITAN ) {
		$optionsurl = add_query_arg( 'page', THEMEPREFIX.'-options', $optionsurl );
	} elseif ( THEMEOPT ) {
		$optionsurl = add_query_arg( 'page', 'options-framework', $optionsurl );
	} else {
		$optionsurl = add_query_arg( 'page', 'theme-info', admin_url( 'themes.php' ) );
	}
	...
	if ( '' != $updated ) {
		$optionsurl = add_query_arg( 'updated', $updated, $optionsurl );
	}

	// --- redirect and exit ---
	wp_redirect( $optionsurl );
	exit;
 }
}
...
if ( !function_exists( 'bioship_admin_options_default_submenu' ) ) {
 ...
 function bioship_admin_options_default_submenu( $menu ) {
	...
	// --- Options Framework only ---
	if ( !THEMEOPT ) {
	    // 2.2.0: added missing return argument
		return $menu;
	}
...
if ( !function_exists( 'bioship_admin_theme_options_advanced' ) ) {
 ...
 add_action( 'admin_menu', 'bioship_admin_theme_options_advanced', 20 );
 ...
 function bioship_admin_theme_options_advanced() {
	...
	if ( $splitoptions ) {
		...
		$advanced_url = add_query_arg( 'options', 'advanced', admin_url( 'customize.php' ) );
		$advanced_url = add_query_arg( 'return', urlencode( $current_url ), $advanced_url);
		if ( isset( $_REQUEST['theme'] ) ) {
			$advanced_url = add_query_arg( 'theme', $_REQUEST['theme'], $advanced_url );
		}
		add_theme_page( __( 'Customize Advanced', 'bioship' ), __( 'Advanced Options', 'bioship' ), 'edit_theme_options', $advanced_url );
	}
 }
}
 ...
 function bioship_admin_theme_options_position() {
	...
	if ( isset( $submenu['themes.php'] ) ) {
		foreach ( $submenu['themes.php'] as $submenukey => $values ) {
			...
			if ( 'themes.php' == $pageslug ) {
				$themesposition = $submenukey;
				$themessubmenu = $values;
			}
			...
			if ( ( 'options-framework' == $pageslug ) || ( strstr( $pageslug, 'page=options-framework' ) )
			  || ( 'theme-options' == $pageslug ) || ( strstr( $pageslug, 'page=theme-options' ) ) ) {
		...
		if ( $dosplice ) {
			...
			foreach ( $submenuthemesb as $key => $value ) {
				$newthemesa[( $key + 1 )] = $value;
			}
...
if ( !function_exists('bioship_admin_themetestdrive_options' ) ) {
 function bioship_admin_themetestdrive_options() {
	...
	foreach ( $submenu['themes.php'] as $submenukey => $values ) {
		// if ( THEMEDEBUG ) {
		//	echo 'Theme Submenu Item: ' . print_r( $values, true ) . PHP_EOL;
		// }
		$value = $submenu['themes.php'][$submenukey][2];
		// --- hack Titan Theme Options submenu URL ---
		if ( 'theme-options' == $value ) {
			$submenu['themes.php'][$submenukey][2] = 'themes.php?page=theme-options&theme=' . esc_attr( $tdtheme );
			break;
		}
		// --- hack Options Framework submenu URL ---
		if ( 'options-framework' == $value ) {
			$submenu['themes.php'][$submenukey][2] = 'themes.php?page=options-framework&theme=' . esc_attr( $tdtheme );
			break;
		}
		// --- hack Customizer Theme Test drive URLs ---
		if ( strstr( $value, 'customize.php' ) ) {
			$submenu['themes.php'][$submenukey][2] = add_query_arg( 'theme', esc_attr( $tdtheme ), $value );
		}
	}
	...
	foreach ( $menu as $priority => $values ) {
		...
		if ($values[2] == $optionsmenukey) {
			$menu[$priority][2] = 'admin.php?page=' . esc_attr( $optionsmenukey ) . '&theme=' . esc_attr( $tdtheme );
			break;
		}
	}
	...
	bioship_debug( "TestDrive Admin Menu", $menu );
	bioship_debug( "TestDrive Admin SubMenu", $submenu );
...
if ( !function_exists( 'bioship_admin_documentation_shift' ) ) {
 ...
 function bioship_admin_documentation_shift() {
	...
	// --- only for Titan Framework ---
 	// 2.0.9: also no need to check this for Wordpress.org version
	if ( THEMEWPORG || !THEMETITAN ) {
		return;
	}
	...
	if ( ( array_key_exists( 'bioship-options', $submenu ) )
	  && ( array_key_exists( 'themes.php', $submenu ) ) ) {
		foreach ( $submenu[THEMEPREFIX . '-options'] as $key => $values ) {
			...
			if ( THEMEPREFIX . '-options-wp-support-forum' == $values[2] ) {
				$values[0] = "<div id='" . esc_attr( THEMEPREFIX ) . "-support-link'>" . $values[0] . "</div>";
		...
		foreach ( $submenu['themes.php'] as $key => $values ) {
			if ( $values[2] == THEMEPREFIX . '-documentation' ) {
				...
				$values[0] = "<div id='" . esc_attr( THEMEPREFIX ) . "-doc-link'>" . $values[0] . "</div>";
...
if ( isset( $_REQUEST['page'] ) && function_exists( 'add_action' ) ) {
	...
	if ( 'bioship-options' == $_REQUEST['page'] ) {
		...
		add_action( 'admin_enqueue_scripts', 'bioship_admin_style_code_editor' );
	}

	// --- Add Top Menu section to Admin Notices [Options Framework] ---
	if ( 'options-framework' == $_REQUEST['page'] ) {
	...
	if ( THEMEPREFIX . '-options-wp-support-forum' == $_REQUEST['page'] ) {
		if ( !function_exists( 'wp_redirect' ) ) {
			include ABSPATH . WPINC . '/pluggable.php';
		}
		wp_redirect( THEMESUPPORT . '/quest-category/theme-support/' );
		exit;
	}
	...
	// 2.0.9: moved here from inside Freemius init function
	if ( THEMEPREFIX.'-documentation' == $_REQUEST['page'] ) {
		if ( !function_exists( 'wp_redirect' ) ) {
			include ABSPATH . WPINC . '/pluggable.php';
		}
		wp_redirect( THEMEHOMEURL.'/documentation/' );
		exit;
	}
...
// ------------------------------
// Enqueue Code Editor for Styles
// ------------------------------
// 2.2.0: enqueue code editor for admin style boxes
if ( !function_exists( 'bioship_admin_style_code_editor' ) ) {
 function bioship_admin_style_code_editor() {
	...
	add_action( 'admin_footer', 'bioship_admin_initialize_code_editor' );
	
 }
}

// ---------------------------------
// Initialize Code Editor for Styles
// ---------------------------------
// 2.2.0: initialize code editor for admin style boxes
if ( !function_exists( 'bioship_admin_initialize_code_editor' ) ) {
 function bioship_admin_initialize_code_editor() {
	echo "<script>jQuery(document).ready(function() {
		/* intialize style textareas */
		setTimeout(function() {
			if (jQuery('#bioship_dynamiccustomcss').length) {wp.codeEditor.initialize(jQuery('#bioship_dynamiccustomcss'), bioship_style_settings);}
			if (jQuery('#bioship_dynamicadmincss').length) {wp.codeEditor.initialize(jQuery('#bioship_dynamicadmincss'), bioship_style_settings);}
		}, 500);
		/* refresh areas on switching to tabs */
		jQuery('.nav-tab').click(function() {
			title = jQuery(this).attr('title');
			if (title == 'Styles') {jQuery('#bioship_dynamiccustomcss').parent.find('.CodeMirror')[0].CodeMirror.refresh();}
			else if (title == 'Admin') {jQuery('#bioship_dynamicadmincss').parent.find('.CodeMirror')[0].CodeMirror.refresh();}
		});
	});</script>";
 }	
}
...
if ( !function_exists( 'bioship_admin_theme_options_page' ) ) {
 function bioship_admin_theme_options_page() {
	...
	if ( THEMETITAN ) {
		$actionurl = add_query_arg( 'page', THEMEPREFIX.'-options', $actionurl );
	} else {
		$actionurl = add_query_arg( 'page', 'options-framework', $actionurl );
	}
	...
	if ( isset( $_REQUEST['newchildname'] ) || isset( $_REQUEST['newclonename'] ) ) {
		...
		$method = '';
		$context = false;
		$extrafields = array( 'newchildname' ); // not sure if this is really needed
	...
	echo '<br><div id="themeoptionsheader"><table><tr>';
		...
		if ( $themelogo ) {
			...
			echo '<td style="vertical-align:top;">';
				echo '<img src="' . esc_url( $themelogo ) . '">';
			echo '</td>';
		}
		echo '<td width="10"></td>';

		// Theme Name / Version / Links
		// ----------------------------
		echo '<td style="vertical=align:top;">';
			...
			echo '<table id="themedisplayname" cellpadding="0" cellspacing="0"><tr height="40">';
				...
				echo '<td style="vertical-align:middle;">';
					echo '<h2 style="margin:5px 0;">' . esc_html( THEMEDISPLAYNAME ) . '</h2>';
				echo '</td>';
				...
				if ( THEMECHILD ) {
					echo '<h3 style="margin:5px 0;">v' . esc_attr( THEMECHILDVERSION ) . '</h3>';
				} else {
					echo 'v' . esc_attr( THEMEVERSION );
				}
				echo '</td></tr>';
				...
				if ( THEMECHILD ) {
					...
					echo esc_html( __( 'Parent Theme', 'bioship' ) ) . ': ';
					echo esc_html( __( 'BioShip Framework', 'bioship' ) );
				...
				echo '<a href="' . esc_url( THEMEHOMEURL . '/news/' ) . '" class="frameworklink" title="' . esc_attr( 'BioShip Theme Framework News', 'bioship' ) . '" target="_blank">';
					echo esc_html( __( 'News', 'bioship' ) ) . '</a> | ';
				echo '<a href="' . esc_url( THEMEHOMEURL . '/documentation/' ) . '" class="frameworklink" title="' .esc_attr( 'BioShip Theme Framework Documentation', 'bioship' ) . '" target="_blank">';
					echo esc_html( __( 'Docs', 'bioship' ) ) . '</a> | ';
				echo '<a href="' . esc_url( THEMEHOMEURL . '/development/' ) . '" class="frameworklink" title="' . esc_attr( 'BioShip Theme Framework Development', 'bioship' ) . '" target="_blank">';
					echo esc_html( __( 'Dev', 'bioship' ) ) . '</a>';
				// echo ' | <a href="' . esc_url( THEMEHOMEURL . '/extensions/' ) . '" class="frameworklink" title="' . esc_attr( 'BioShip Theme Framework Extensions', 'bioship' . '" target="_blank">';
					// echo esc_html( __( 'Extend','bioship' ) ) . '</a>';
				echo '</font></center></td>';
		...
		// --- create child theme / child theme clone ---
		...
		if ( isset( $_REQUEST['newchildname'] ) || isset( $_REQUEST['newclonename'] ) ) {
			...
			if ( isset( $_REQUEST['newchildname'] ) ) {
				check_admin_referer( 'bioship_child_install' );
			} elseif ( isset( $_REQUEST['newclonename'] ) ) {
				check_admin_referer( 'bioship_child_clone' );
			}
			...
			if ( $filesystemcheck ) {
				...
				$message = __( 'Check your file system owner/group permissions!', 'bioship' );
			}
			if ( '' != $message ) {
        ...
        // 2.2.0: fix for possible undefined variable warning
        $newchildname = $newclonename = '';
		if ( !THEMECHILD ) {
			$newchildname = "BioShip Child";
		} else {
			$newclonename = THEMEDISPLAYNAME . ' Clone';
		}

		// 2.1.1: use THEMECHILD constant instead of is_child_theme()
		if ( !THEMECHILD && !$newchild ) {

			if ( isset( $_REQUEST['newchildname'] ) && ( '' != trim( $_REQUEST['newchildname'] ) ) ) {
			...
			if ( isset( $_REQUEST['theme'] ) && ( '' != trim( $_REQUEST['theme'] ) ) ) {
				...
				echo '<input type="hidden" name="theme" value="' . esc_attr( $theme ) . '">';
			}
			echo '<table><tr><td align="left">';
				echo '<font style="font-size:11pt;line-height:22px;"><b>' . esc_html( __( 'Create Child Theme', 'bioship' ) ) . ':</b></font>';
			echo '</td><td width="10"></td><td>';
				echo '<input type="text" name="newchildname" style="font-size:11pt; width:120px;" value="' . esc_attr( $newchildname ) . '">';
			echo '</td><td width="10"></td><td>';
				echo '<input type="submit" class="button-primary" title="' . esc_attr( __( 'Note: alphanumeric and spaces only.', 'bioship' ) ) . '" value="' . esc_attr( __( 'Create', 'bioship' ) ) . '"></td>';
			echo '</td></tr></table></form>';

		} elseif ( THEMECHILD && !$newclone ) {

			if ( isset($_REQUEST['newclonename'] ) && ( '' != trim( $_REQUEST['newclonename'] ) ) ) {
			...
			echo '<table><tr><td align="left">';
				echo '<font style="font-size:11pt;line-height:22px;"><b>' . esc_html( __( 'Clone Child Theme to', 'bioship' ) ) . ':</b></font>';
			echo '</td><td width="10"></td><td>';
				echo '<input type="text" name="newclonename" style="font-size:11pt; width:120px;" value="' . esc_attr( $newclonename ) . '">';
			echo '</td><td width="10"></td><td>';
				echo '<input type="submit" class="button-primary" title="' . esc_attr( __( 'Note: alphanumeric and spaces only.', 'bioship' ) ) . '" value="' . esc_attr( __( 'Clone', 'bioship' ) ) . '">';
	...
	ob_start();
	bioship_do_action( 'theme_admin_notice' );
	$themenotices = ob_get_contents();
	ob_end_clean();
	if ( '' != $themenotices ) {
		echo '<div style="float:none; clear:both;"></div><br>';
		echo '<div class="' . esc_attr( $nagclass ) . '" style="padding:3px 10px;margin:0 0 10px 0;">';
		// phpcs:ignore WordPress.Security.OutputNotEscaped,WordPress.Security.OutputNotEscapedShortEcho
		echo $themenotices . '</div>';
	}

	// Theme Info Section
	// ------------------
	echo '<div id="extendwrapper" class="wrap"';
	if ( '' != $vthemesettings['layertab'] ) {
...
if ( !function_exists( 'bioship_admin_theme_options_scripts' ) ) {
 function bioship_admin_theme_options_scripts() {
	...
	// - for Titan Framework -
	if ( THEMETITAN ) {
		$tableclasses = '.titan-framework-panel-wrap,.nav-tab-wrapper,.options-container,.form-table';
	}
	...
	echo "function resizeoptionstables() {
		if (document.getElementById('floatdiv')) {
			...
			if (newwidth < 640) {
				...
				jQuery('" . esc_js( $tableclasses ) . "').css('width','100%');
			} else {
				document.getElementById('floatdiv').style.display = 'block';
				jQuery('" . esc_js( $tableclasses ) . "').css('width',newwidth+'px');
			}
	...
	if ( THEMETITAN ) {
		echo PHP_EOL . "var dynamiccssareaid = '" . esc_js( $vthemename ) . "_dynamiccustomcss';" . PHP_EOL;
	} else {
		echo PHP_EOL . "var dynamiccssareaid = 'dynamiccustomcss';" . PHP_EOL;
	}
	...
	echo "jQuery(document).ready(function($) {
		...
		// echo "if (typeof stick_in_parent === 'function') {";
			echo "if (jQuery('#floatdiv').length) {
				jQuery('#floatdiv').stick_in_parent({offset_top:100});
			}" . PHP_EOL;
		// echo "}";
...
if ( !function_exists( 'bioship_admin_theme_options_styles' ) ) {
 ...
 function bioship_admin_theme_options_styles() {
	...
	// 2.2.0: load styles on theme tools page also
	global $vthemename;
	$valid = array('options-framework', 'bioship-options', $vthemename . '-options', 'theme-info');
	if ( !isset( $_REQUEST['page'] ) || !in_array( $_REQUEST['page'], $valid ) ) {
		return;
	}
...
if ( !function_exists( 'bioship_admin_floating_sidebar' ) ) {
 function bioship_admin_floating_sidebar() {
	...
	if ( $wordquest ) {
		include_once $wordquest;
		wqhelper_admin_loader();
	}
	...
	if ( !function_exists( 'bioship_admin_sidebar_save_button' ) ) {
	 ...
	 function bioship_admin_sidebar_save_button( $button ) {
		...
		// 2.2.0: change input tag to button tag to prevent stripping
		$button = '<table><tr><td align="center">';
		$button .= '<button id="sidebarsavebutton" type="button" class="button-primary">' . esc_html( __( 'Save Settings', 'bioship' ) ) . '</button></td>';
		$button .= '<td width="30"></td><td>';
			$button .= '<div style="line-height:1em;"><font style="font-size:8pt;">';
			$button .= '<a href="javascript:void(0);" id="sidebaroptionsbutton" style="text-decoration:none;">';
				$button .= esc_html( __( 'Sidebar', 'bioship' ) ) . '<br>' . esc_html( __( 'Options', 'bioship' ) );
			$button .= '</a></font></div>';
		$button .= '</td></tr></table>';
...
if ( !function_exists( 'bioship_admin_quicksave_css' ) ) {
 ...
 function bioship_admin_quicksave_css() {
	...
	if ( current_user_can( 'edit_theme_options' ) ) {
		...
		// TODO: maybe trigger interim login box popover ?
...
if ( !function_exists( 'bioship_theme_options_refresh_titan_nonce' ) ) {
 ...
 function bioship_theme_options_refresh_titan_nonce() {
	...
	// 2.2.0: check edit_theme_options permissions to be safe
	if ( current_user_can( 'edit_theme_options' ) ) {
		...
		echo "<script>parent.document.getElementById('titan-framework_nonce').value = '" .esc_js( $titannonce ) . "';
		console.log('Titan Admin Page Nonce Refreshed: " . esc_js( $titannonce ) . "');</script>";
	}
	exit;
 }
}
...
if ( !function_exists( 'bioship_admin_theme_info_page' ) ) {
 function bioship_admin_theme_info_page() {
	...
	echo "<h3>" . esc_html( __( 'BioShip Theme Info', 'bioship' ) ) . "</h3><br>";
...
if ( !function_exists( 'bioship_admin_theme_info_section' ) ) {
 function bioship_admin_theme_info_section() {
	...
	echo "<script>function togglethemebox(divid) {
		arrowid = divid+'-arrow'; divid += '-inside';
	...
	// --------------------------------
	$boxid = 'adminnotices';
	$boxtitle = __( 'Admin Notices', 'bioship' );
	// 2.2.0: exclude if adminsanity notice boxer is loaded
	if ( !defined( 'ADMINSANITY_LOAD_NOTICES' ) || !ADMINSANITY_LOAD_NOTICES ) {
		...
		echo '<h3 class="hndle" onclick="togglethemebox(\'' . esc_attr( $boxid ) . '\');">';
		echo '<span id="' . esc_attr( $boxid ) . '-arrow">&#9662;</span> ' . esc_html( $boxtitle ) . '</h3>';
		echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside" style="display:none;">';
		// TODO: maybe use wqhelper admin notices boxer instead ?
		echo '<h2></h2>';
	...
	// ------------------------------
	// (collapsed unless welcome page)
	// 2.0.5: add combined welcome and documentation box
	$boxid = 'documentation';
	if ( $welcome ) {
		$boxtitle = __( 'Welcome!', 'bioship' );
	} else {
		$boxtitle = __( 'Documentation', 'bioship' );
	}
	echo '<div id="' . esc_attr( $boxid ) . '" class="postbox" style="max-width:680px;">';
	...
	echo '</span> ' . esc_html( $boxtitle ) . '</h3>';
	...
	// ------------------------------
	$boxid = 'themetools';
	$boxtitle = __( 'Theme Settings Tools', 'bioship' );
	echo '<div id="' . esc_attr( $boxid ) . '" class="postbox" style="max-width:680px;">';
	echo '<h3 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');">';
	echo '<span id="' . esc_attr( $boxid ) . '-arrow">&#9662;</span> ' . esc_html( $boxtitle ) . '</h3>';
	...
	echo '<div id="extendcolumn">';
		...
		// -------------------
		$boxid = 'bioshiplinks';
		$boxtitle = __( 'Theme Links','bioship' );
		echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
		echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
		echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
			...
			echo '<td width="20"></td>';
			echo '<td><a href="' . esc_url( THEMEHOMEURL . '/documentation/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'BioShip Documentation', 'bioship' ) ) . '</a><br>';
			// TODO: maybe add a dropdown list of documentation subpages?
			// --- Support Forum Links ---
			// 2.1.1: fix to support forum link URLs
			echo '<a href="' . esc_url( THEMESUPPORT . '/quest-category/bioship-support/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'Support Solutions', 'bioship' ) ) . '</a><br>';
			echo '<a href="' . esc_url( THEMESUPPORT . '/quest-category/bioship-features/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'Features and Feedback', 'bioship' ) ) . '</a><br>';
			// --- Development ---
			echo '<a href="https://github.com/majick777/bioship/" class="themelink" target="_blank">' . esc_html( __( 'Development via GitHub', 'bioship' ) ) . '</a><br><br>';
			// --- Extensions ---
			echo '<a href="' . esc_url( THEMEHOMEURL . '/extensions/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'BioShip Extensions', 'bioship' ) ) . '</a><br>';
			// --- Content Sidebars / AutoSave Net / ...
			echo '&rarr; <a href="' . esc_url( THEMESUPPORT . '/plugins/content-sidebars/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'Content Sidebars Plugin', 'bioship' ) ) . '</a><br>';
		...
		// WordQuest Plugins
		// -----------------
		$boxid = 'wordquestplugins';
		$boxtitle = __( 'WordQuest Alliance', 'bioship' );
		echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
		echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
		echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
			$wordquestlogo = bioship_file_hierarchy( 'url', 'wordquest.png', $vthemedirs['image'] );
			echo '<table><tr><td><a href="' . esc_url( THEMESUPPORT ) . '" target=_blank><img src="' . esc_url( $wordquestlogo ) . '" border="0"></a></td><td width="20"></td><td>';
			if ( isset( $GLOBALS['admin_page_hooks']['wordquest'] ) ) {
				...
				echo '<a href="' . esc_url( $wqpanel ) . '" class="themelink">' . esc_html( __( 'Plugin Panel', 'bioship' ) ) . '</a><br>';
			}
			echo '<a href="' . esc_url( THEMESUPPORT . '/register/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'Join the Alliance', 'bioship' ) ) . '</a><br>';
			echo '<a href="' . esc_url( THEMESUPPORT . '/login/' ) . '" class="themelink" target="_blank">' . esc_html( __( 'Members Login', 'bioship' ) ) . '</a><br>';
			echo '<a href="' . esc_url( THEMESUPPORT . '/solutions/' ) . '" class="themelink" target=_blank>' . esc_html( __( 'Solutions Forum', 'bioship' ) ) . '</a><br>';
			echo '<a href="' . esc_url( THEMESUPPORT . '/plugins/' ) . '" class="themelink" target=_blank>'.esc_html( __( 'WordQuest Plugins', 'bioship' ) ) . '</a><br>';
			echo '</td></tr></table>';
		echo '</div></div>';
		...
		if ( $recommended ) {
			...
			if ( $recommend ) {
				$boxid = 'recommended';
				$boxtitle = __( 'Recommended', 'bioship' );
				echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
				echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
				echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
					// phpcs:ignore WordPress.Security.OutputNotEscaped,WordPress.Security.OutputNotEscapedShortEcho
	...
	echo '<div id="feedcolumn">';
		...
		if ( isset( $_REQUEST['loadfeeds'] ) ) {
			...
			if ( ( 'on' == $loadfeeds ) || ( '1' == $loadfeeds ) ) {
				delete_option( 'bioship_admin_feed_display' );
			}
			if ( ( 'off' == $loadfeeds ) || ( '0' == $loadfeeds ) ) {
				update_option( 'bioship_admin_feed_display', 'off' );
			}
			if ( ( 'no' == $loadfeeds ) || ( '2' == $loadfeeds ) ) {
				$feeds = false;
			}
		}
		$feeddisplay = get_option( 'bioship_admin_feed_display' );
		if ( 'off' == $feeddisplay ) {
			$feeds = false;
		}
		...
		if ( $feeds ) {
			...
			// BioShip Feed
			// ------------
			// 2.2.0: moved and prefixed muscle_bioship_dashboard_feed_widget to admin.php
			if ( function_exists( 'bioship_muscle_bioship_dashboard_feed_widget' ) ) {
				$boxid = 'bioshipfeed';
				$boxtitle = __( 'BioShip News', 'bioship' );
				echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
				echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
				echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
					echo "<!-- start BioShip News Feed -->";
						bioship_muscle_bioship_dashboard_feed_widget( false, false );
			...
			if ( function_exists( 'wqhelper_dashboard_feed_widget' ) ) {
				$boxid = 'wordquestfeed';
				$boxtitle = __( 'WordQuest News', 'bioship' );
				echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
				echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_attr( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
				echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
			...
			if ( function_exists( 'wqhelper_pluginreview_feed_widget' ) ) {
				...
				$boxtitle = __( 'Plugin Reviews', 'bioship' );
				echo '<div id="' . esc_attr( $boxid ) . '" class="postbox">';
				echo '<h2 class="hndle" onclick="togglethemebox(\'' . esc_js( $boxid ) . '\');"><span>' . esc_html( $boxtitle ) . '</span></h2>';
				echo '<div class="inside" id="' . esc_attr( $boxid ) . '-inside">';
...
if ( !function_exists( 'bioship_admin_welcome_message' ) ) {
 function bioship_admin_welcome_message() {
	...
	$message = '<p align="left">' . esc_html( __( 'Welcome aboard', 'bioship' ) ) . ' ' . esc_attr( $firstname ) . '! ';
	$message = esc_html( __( 'And thanks for choosing to pilot BioShip...', 'bioship' ) ) . '</p>';
...
if ( !function_exists( 'bioship_admin_documentation_box' ) ) {
 function bioship_admin_documentation_box( $welcome ) {
	...
	echo '>';
		echo '<h4>' . esc_html( __( 'QuickStart Guide', 'bioship' ) ) . '</h4>';
	...
	$quickstart .= esc_html( __( 'Show QuickStart Guide', 'bioship' ) ) . '</a></div>';
	...
	$quickstart .= esc_html( __( 'Hide QuickStart Guide', 'bioship' ) ) . '</a></div>';
	$quickstart .= '<br>';
...
if ( !function_exists( 'bioship_admin_build_selective_resources' ) ) {
 ...
 function bioship_admin_build_selective_resources() {
	...
	$dobuild = false;
	if ( 'customize_save_after' == $currentaction ) {
		$dobuild = true;
	} elseif ( 'admin_notices' == $currentaction ) {

		// --- Options Framework ---
		if ( isset( $_GET['page'] ) && isset( $_GET['settings-updated'] ) ) {
			if ( ( 'options-framework' == $_GET['page'] ) && ( 'true' == $_GET['settings-updated'] ) ) {
				$dobuild = true;
			}
		}
		// --- Titan Framework ---
		...
		if ( isset( $_GET['page'] ) && isset( $_GET['message'] ) ) {
			if ( ( $vthemename . '-options' == $_GET['page'] ) && ( 'saved' == $_GET['message'] ) ) {
				$dobuild = true;
			}
		}
	}
	...
	if ( $vthemesettings['combinecsscore'] ) {
		...
		if ( 'normalize' == $resetoption ) {
			$cssfile = bioship_file_hierarchy( 'file', 'normalize.css', $vthemedirs['style'] );
			if ( $cssfile ) {
				$cssreset = bioship_file_get_contents( $cssfile );
			} else {
				echo '<b>' . esc_html( __( 'Warning', 'bioship' ) ) . '</b>: ';
				echo esc_html( __( 'CSS Combine could not find', 'bioship' ) ) . ' <i>normalize.css</i><br>';
			}
		}
		if ( 'reset' == $resetoption ) {
			$cssfile = bioship_file_hierarchy( 'file', 'reset.css', $vthemedirs['style'] );
			if ( $cssfile ) {
				$cssreset = bioship_file_get_contents( $cssfile );
			} else {
				echo '<b>' . esc_html( __( 'Warning','bioship' ) ) . '</b>: ';
				echo esc_html( __( 'CSS Combine could not find', 'bioship' ) ) . ' <i>reset.css</i><br>';
			}
		}
		...
		if ( $vthemesettings['loadformalize'] ) {
			...
			if ( $cssfile ) {
				$formalize = bioship_file_get_contents( $cssfile );
			} else {
				echo '<b>' . esc_html( __( 'Warning', 'bioship' ) ) . '</b>: ';
				echo esc_html( __( 'CSS Combine could not find', 'bioship' ) ) . ' <i>formalize.css</i><br>';
			}
		}
		...
		if ( $cssfile ) {
			$mobile = bioship_file_get_contents( $cssfile );
		} else {
			echo '<b>' . esc_html( __( 'Warning', 'bioship' ) ) . '</b>: ';
			echo esc_html( __( 'CSS Combine could not find', 'bioship' ) ) . ' <i>mobile.css</i><br>';
		}
		...
		if ( $cssfile ) {
			$skeleton = bioship_file_get_contents( $cssfile );
		} else {
			echo '<b>' . esc_html( __( 'Warning', 'bioship' ) ) .'</b>: ';
			echo esc_html( __( 'CSS Combine could not find', 'bioship' ) ) . ' <i>skeleton.css</i><br>';
		}
		...
		foreach ( $vthemedirs['style'] as $dir ) {
			if ( is_dir( $stylepath . DIRSEP . $dir ) && file_exists( $stylepath . DIRSEP . $dir . DIRSEP . 'core-styles.css' ) ) {
			    // 2.2.0: fix to unnecessary concatenation to stylefile
				$stylefile = $stylepath . DIRSEP . $dir . DIRSEP . 'core-styles.css';
				break;
			}
		}
		if ( isset( $stylefile ) ) {
			bioship_write_to_file( $stylefile, $csscontents );
		}
	}
	...
	if ( 'selective' == $vthemesettings['loadfoundation'] ) {

		// 2.2.0: use scripts not javascripts directory
		$jsfile = bioship_file_hierarchy( 'file', 'foundation.js', array( 'scripts','includes/'.$foundation.'/js/foundation' ) );
		...
		foreach ( $selected as $key => $value ) {
			if ( '' == $value ) {
				...
				if ( $jsfile ) {
					...
					$message .= "<b>" . esc_html( __( 'Warning', 'bioship' ) ) . '</b>: ';
					$message .= esc_html( __( 'Foundation JS Combine could not find', 'bioship' ) ) . " <i>" . esc_attr( $filename ) . "</i><br>";
				}
			}
		}
		...
		if ( strlen( $foundation ) > 0 ) {
			// 1.8.0: write to theme scripts directory, not foundation/js and fix directory separator
			...
			foreach ( $vthemedirs['script'] as $dir ) {
				if ( is_dir( $scriptpath . DIRSEP . $dir ) && file_exists( $scriptpath . DIRSEP . $dir . DIRSEP . 'foundation.selected.js' ) ) {
				  	$scriptfile = $scriptpath . DIRSEP . $dir . DIRSEP . 'foundation.selected.js';
				  	break;
				  }
			}
			if ( isset( $scriptfile ) ) {
				bioship_write_to_file( $scriptfile, $foundationjs );
			}
		}
	}
...
if ( !THEMECHILD ) {
	...
	// ----------------------------------
	// 2.2.0: added to prevent empty settings on first use
	if ( !function_exists( 'bioship_set_default_settings' ) ) {

	 add_action( 'after_switch_theme', 'bioship_set_default_settings' );

	 function bioship_set_default_settings( $force = false ) {
		$settings = get_option( THEMEKEY, array() );
		if ( $force || ( null === $settings ) ) {
			$settings = array();
			// 2.2.0: inclue options.php file here
			...
			wp_cache_delete( THEMEKEY, 'options' );
			remove_filter( 'pre_update_option', 'bioship_admin_theme_settings_save', 11, 3 );
			update_option( THEMEKEY, $settings );
			add_filter( 'pre_update_option', 'bioship_admin_theme_settings_save', 11, 3 );
		}
	 }
	}

	// --- check save restore widgets filter ---
	...
	if ( $saverestorewidgets ) {
		...
		if ( !function_exists( 'bioship_admin_theme_deactivation' ) ) {
		 ...
		 function bioship_admin_theme_deactivation( $newthemename ) {

			// 2.2.0: fix to use THEMEPREFIX constant
			$sidebarswidgets = get_option( 'sidebars_widgets' );
			update_option( THEMEPREFIX . '_widgets_backup', $sidebarswidgets );
			$menusettings = get_option( 'nav_menu_options' );
			update_option( THEMEPREFIX . '_menus_backup', $menusettings );
		...
		if ( !function_exists( 'bioship_admin_theme_activation' ) ) {
		 ...
		 function bioship_admin_theme_activation() {
			...
			if ( '' != $bioshipwidgets ) {
				...
				update_option( 'old_theme_widgets_backup', $sidebarswidgets );
			}
			if ( '' != $bioshipmenus ) {
				update_option( 'nav_menu_options', $bioshipmenus );
				update_option( 'old_theme_menus_backup', $menusettings );
			}
			...
			if ( is_admin() && isset( $_GET['activated'] ) && ( 'themes.php' == $pagenow ) ) {
				...
				// 2.2.0: use add_query_arg to generate welcome URLs
				if ( !THEMETITAN && THEMEOPT ) {
					$location = add_query_arg( 'page', 'options-framework', admin_url( 'themes.php' ) );
					$location = add_query_arg( 'welcome', 'true', $location );
				} elseif ( THEMETITAN || class_exists( 'TitanFramework' ) ) {
					$location = add_query_arg( 'page', THEMEPREFIX . '-options', admin_url( 'admin.php' ) );
					$location = add_query_arg( 'welcome', 'true', $location );
				}
				// else {
				//	2.0.8: WordPress.Org versions only - redirection on theme activation not allowed :-/
				//	$location = add_query_arg( 'page', 'theme-info', admin_url( 'themes.php' );
				//	$location = add_query_arg( 'welcome', 'true', $location );
				// }
	...
	if ( $saverestorechildwidgets ) {
		...
		if ( !function_exists( 'bioship_admin_get_child_theme_slug'  )) {
		 function bioship_admin_get_child_theme_slug() {
			$theme = wp_get_theme();
			if ( !THEMETITAN && THEMEOPT ) {
				$childthemeslug = preg_replace( "/\W/", "_", strtolower( $theme['Name'] ) );
			} else {
				$childthemeslug = preg_replace( "/\W/", "-", strtolower( $theme['Name'] ) );
			}
		...
		if ( !function_exists( 'bioship_admin_child_theme_deactivation' ) ) {
		 ...
		 function bioship_admin_child_theme_deactivation( $newthemename ) {
			...
			update_option( $childthemeslug . '_widgets_backup', $sidebarswidgets );
			$menusettings = get_option( 'nav_menu_options' );
			delete_option( $childthemeslug . '_menus_backup' );
			update_option( $childthemeslug . '_menus_backup', $menusettings );
		 }
		}
		...
		if ( !function_exists( 'bioship_admin_child_theme_activation' ) ) {
		 ...
		 function bioship_admin_child_theme_activation() {
			...
			if ( '' != $backupwidgets ) {
				update_option( 'sidebars_widgets', $backupwidgets );
			}
			if ( '' != $backupmenus ) {
				update_option( 'nav_menu_options', $backupmenus );
			}
			...
			if ( is_admin() && isset( $_GET['activated'] ) && ( 'themes.php' == $pagenow ) ) {
				if ( !THEMETITAN && THEMEOPT ) {
					$location = add_query_arg( 'page', 'options-framework', admin_url( 'admin.php' ) );
				} elseif ( THEMETITAN || class_exists('TitanFramework' ) ) {
					$location = add_query_arg( 'page', THEMEPREFIX . '-options', admin_url( 'admin.php' ) );
				}
...
// TODO: fix or deprecate? automatic theme mods transfer ?
...
if ( !function_exists( 'bioship_admin_install_titan_framework' ) ) {
 ...
 function bioship_admin_install_titan_framework() {
	...
	if ( 'yes' != $_REQUEST['admin_install_titan_framework'] ) {
		return;
	}
...
if ( function_exists( 'tgmpa' ) ) {
	...
	if ( !function_exists( 'bioship_admin_tgm_titan_framework_check' ) ) {
	 function bioship_admin_tgm_titan_framework_check( $plugins ) {
	 	...
	 	if ( !THEMEWPORG || THEMETITAN || class_exists( 'TitanFramework' ) ) {
	 		return $plugins;
	 	}
    ...
    // 2.2.0: added missing global declaration to get wp_version
    global $wp_version;
	...
	if ( isset( $_REQUEST['page'] ) ) {
		...
		if ( ( 'options-framework' == $_REQUEST['page'] )
		  || ( THEMESLUG . '-options' == $_REQUEST['page'] )
		  || ( THEMESLUG . '_options' == $_REQUEST['page'] ) ) {
	...
	if ( !function_exists( 'bioship_admin_tgm_notice_shift' ) ) {
	 function bioship_admin_tgm_notice_shift() {
		...
		if ( get_user_meta( $userid, 'tgmpa_dismissed_notice_' . $id, true ) ) {
			add_user_meta( $userid, 'tgmpa_temp_notice_' . $id, 1 );
			delete_user_meta( $userid, 'tgmpa_dismissed_notice_' . $id );
	...
	if ( !function_exists( 'bioship_admin_tgm_notice_unshift' ) ) {
	 function bioship_admin_tgm_notice_unshift() {
		...
		delete_user_meta( $userid, 'tgmpa_temp_notice_' . $id );
		add_user_meta( $userid, 'tgmpa_dismissed_notice_' . $id, 1 );
	 }
	}
	...
	if ( !function_exists( 'bioship_admin_tgm_dismiss_notice_off' ) ) {
	 function bioship_admin_tgm_dismiss_notice_off( $config ) {
		...
		$message = '<h3>' . esc_html( __( 'BioShip Theme Recommended Plugins', 'bioship' ) ) . '</h3><br>';
	...
	if ( !function_exists( 'bioship_admin_register_plugins' ) ) {
	 function bioship_admin_register_plugins() {
		...
		$plugins = array(
			...
			// 2.2.0: removed as core handles widgets better now
			// array(
			//	'name'      => 'Widget Saver',
			//	'slug'      => 'widget-saver',
			//	'required'  => false,
			...
			// 2.2.0: added AdminSanity release
		...
		if ( !THEMEWPORG ) {
			foreach ( $plugins as $i => $plugin ) {
				if ( isset( $plugin['wq'] ) && $plugin['wq'] ) {
					$plugins[$i]['source'] = THEMESUPPORT . '/downloads/packages/' . $plugin['slug'] . '.zip';
					$plugins[$i]['external_url'] = THEMESUPPORT . '/plugins/' . $plugin['slug'] . '/';
				}
			}
		}
		...
		$message = '<h3>' . esc_html( __( 'BioShip Theme Framework', 'bioship' ) );
		$message .= ' - ' . esc_html( __( 'Recommended Plugins', 'bioship' ) ) . '</h3><br>';
