<article <?php hybrid_attr( 'post' ); ?>>
	...
	<?php if ( is_singular( get_post_type() ) ) : // If viewing a single post. ?>
		...
		<footer class="entry-footer">
			...
			<?php edit_post_link(); ?>
			<?php hybrid_post_terms( array( 'taxonomy' => 'category', 'text' => __( 'Posted in %s', 'bioship' ), 'before' => '<br />' ) ); ?>
	...
	<?php else : // If not viewing a single post. ?>
		...
		<?php if ( !get_option( 'show_avatars' ) ) : // If avatars are not enabled. ?>
			...
			<footer class="entry-footer">
				<?php hybrid_post_format_link(); ?>
				<time <?php hybrid_attr( 'entry-published' ); ?>><?php echo get_the_date(); ?></time>
				<a class="entry-permalink" href="<?php the_permalink(); ?>" rel="bookmark" itemprop="url"><?php _e( 'Permalink', 'bioship' ); ?></a>
