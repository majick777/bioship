...
if ( is_active_sidebar( 'subtaxonomy' ) ) {
	...
	bioship_do_action( 'bioship_before_subsidebar', 'subtaxonomy', $template );
		...
		echo '<aside ' . hybrid_get_attr( 'sidebar', 'subsidiary', $args ) . '>' . PHP_EOL;
			dynamic_sidebar( 'subtaxonomy' );
		echo '</aside>';
		bioship_html_comment( '/#sidebar-subsidiary' );
		echo PHP_EOL;

	bioship_do_action( 'bioship_after_subsidebar', 'subtaxonomy', $template );
