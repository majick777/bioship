...
class Options_Framework_Admin {
    ...
    function settings_init() {
    	...
    	// Theme MOD: Added filter: 'of_theme_value'
    	delete_option('optionsframework_validated');
        $optionsframework_settings = apply_filters('of_theme_value',get_option('optionsframework'));
		if (isset($_GET['theme'])) {$optionsframework_settings['id'] == $_GET['theme'];}
	 ...
	 function options_page() { ?>
	    ...
	    <div id="optionsframework-metabox" class="metabox-holder">
		    <div id="optionsframework" class="postbox">
				...
				<?php // MOD: for Theme Test Drive save Compatibitility
					if (isset($_REQUEST['theme'])) {echo '<form action="options.php?theme='.$_REQUEST['theme'].'&themedrive=1" method="post">';}
					else {echo '<form action="options.php" method="post">';}
				?>
	...
	function validate_options( $input ) {
		...
		// if (get_option('optionsframework_validated') == '1') {
		// 	global $vtheme, $vthemename;
		//	echo "***".$vthemename."***";
		//	print_r($vtheme);
		//	return get_option($vthemename);
		// }
		...
		add_option('optionsframework_validated','1');

		// write debug data
		// $vdebugfile = get_stylesheet_directory().'/debug1.txt';
		// $vdebugsettings = implode('|||',$clean); $vi = 1;
		// if (file_exists($vdebugfile)) {$vi++; $vdebugfile = str_replace('1.txt',$vi.'.txt',$vdebugfile);}
		// $vfh = fopen($vdebugfile,'w'); fwrite($vfh,$vdebugsettings); fclose($vfh);
	...
	function save_options_notice() {
		if (get_option('optionsframework_validated') == '1') {
			// global $vthemename; return get_option($vthemename);
			add_settings_error( 'options-framework', 'save_options', __( 'Options saved.', 'bioship' ), 'updated fade' );
			delete_option('optionsframework_validated');
			remove_action( 'optionsframework_after_validate', array( $this, 'save_options_notice' ) );
		}
	}
